...
function Review(){
    ...
    const removeReview = useCallback( (key) =>{
    ...
    return(
        <div>
            ...
            <div>
                {reviewList.map((_review, index) =>(<ReviewBox review={_review} key={index} removeReview={removeReview} rvList={reviewList}/>))}
            </div>
        </div>
    );
}

const ReviewBox = React.memo(function ReviewBox({review, removeReview, rvList}){
    const [reviewList, setReviewList] = useState();
    const listHeartChange = (_review) =>{
        setReviewList(rvList.map((rv)=>{
    ...
    return(
        <div className="reviewBox">
            <div><b>{review.id}</b>&nbsp;&nbsp;&nbsp;&nbsp;{review.comment}===={review.key}</div>
            <div>
